...
# Original Author: Johann Faouzi <johann.faouzi@gmail.com>
...
def _digitize(X, bins):
    ...
    if isinstance(bins, tuple):
        X_binned = _digitize_2d(X, list(bins), n_samples, n_timestamps)
    else:
        if bins.ndim == 1:
            X_binned = _digitize_1d(X, list(bins), n_samples, n_timestamps)
        else:
            X_binned = _digitize_2d(X, list(bins), n_samples, n_timestamps)
